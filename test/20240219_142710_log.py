...
↓
あるマスを掘った値に応じて，
1以上：そのマスと共起する数に応じて期待値を上げる
0：そのマスと共起する数に応じて期待値を下げる
↓
...
for _ in range(N*N):
    ...
    coordinate_num_same = Counter(coordinate_include)
    print((i,j), file=sys.stderr)
    print(coordinate_num_same, file=sys.stderr)
    #sys.exit()
    ...
    if resp > 0:
        ...
        for tmp in coordinate_include:
            coordinate_exp_fortune[tmp] = 10 * coordinate_num_same[tmp] * coordinate_exp_fortune[tmp]
        """
        # 含まないマスを下げる
        ...
        # 含むマスを下げる
        for tmp in coordinate_include:
            coordinate_exp_fortune[tmp] = (1 / coordinate_num_same[tmp]) * coordinate_exp_fortune[tmp]
