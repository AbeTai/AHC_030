...
※ここで，占いの結果が0だった場合，0.1をかけるように修正
その上で、期待値高い順に掘る
↓
あるマスを占った値に応じて隣接マスの事後確率を更新
ここで，共起数に応じて隣接マスの値を更新．隣接の中でも適切に順位をつける．
↓
...
for _ in range(N*N):
    ...
    if resp > 0:
        ...
        for n in coordinate_next:
            coordinate_exp_fortune[n] = coordinate_num_same[n] * coordinate_exp_fortune[n] # 共起数に応じて更新するよう変更
        
        for n in coordinate_next_cross:
            coordinate_exp_fortune[n] = coordinate_num_same[n] * coordinate_exp_fortune[n]
