...
matching_cells = [(row, col) for row in range(N) for col in range(N) if fortune_res_idx[row] >= 1.0 or fortune_res_col[col] >= 1.0]
not_matching_cells = [(row, col) for row in range(N) for col in range(N) if fortune_res_idx[row] == 0.0 and fortune_res_col[col] == 0.0]
# print(matching_cells, file=sys.stderr)




# 占いの結果，行列がいずれか0より大きかったマスを探索
has_oil = []
oil_reserves = 0
